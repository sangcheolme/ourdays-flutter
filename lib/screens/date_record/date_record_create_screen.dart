...
class DateRecordCreateScreen extends StatefulWidget {
  final DateTime? initialDate;

  const DateRecordCreateScreen({
    super.key,
    this.initialDate,
  });

  @override
  State<DateRecordCreateScreen> createState() => _DateRecordCreateScreenState();
}

class _DateRecordCreateScreenState extends State<DateRecordCreateScreen> {
  ...
  Future<void> _saveDateRecord() async {
    ...
    try {
      ...
      final dateRecord = DateRecord(
        ...
        createdAt: DateTime.now(),
        updatedAt: DateTime.now(), coupleId: '', createdBy: '',
      );

      final success = await dateRecordProvider.createDateRecord(dateRecord);

      if (!mounted) return;

      if (success) {
        // In a real app, we would also upload media files and add places
        Navigator.pop(context, true);
      } else {
        setState(() {
          _errorMessage = '데이트 기록 저장에 실패했습니다.';
        });
      }
    } catch (e) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('데이트 기록하기'),
        elevation: 0,
        actions: [
          TextButton(
            onPressed: _isLoading ? null : _saveDateRecord,
            child: _isLoading
                ? const SizedBox(
                    width: 20,
                    height: 20,
  ...
  Widget _buildMediaItem(XFile file, int index) {
